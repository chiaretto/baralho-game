...
export function checkInvalidRequestError(result: request.Response, message: string) {
  expect(result.status).toBe(400);
  expect(result.body.type).toEqual('InvalidPayload');
  expect(result.body.message).toEqual(message);
}
  
export function checkBusinessError(result: request.Response, errorResponse: BusinessErrorResponse) {
  checkBusinessErrorWithStatus(422, result, errorResponse);
}
export function checkForbiddenError(result: request.Response, errorResponse: BusinessErrorResponse) {
  checkBusinessErrorWithStatus(403, result, errorResponse);
}

export function checkBusinessErrorWithStatus(statusCode: number, result: request.Response, errorResponse: BusinessErrorResponse) {
...
export function getGamePlayer(room: Room, player?: Player) : GamePlayer | undefined {
